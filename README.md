...
python3の実行環境と併せてmysqlおよびnginxの環境をサクッとdockerで構築します。  

なお、dockerの実行環境は各自で構築してください。  
基本的にwindowsでもmacでももちろんlinuxでも動作するはずです。  

pythonコンテナでデータ分析をして結果をHTMLでマークアップしたデータを作成してそれをnginxで閲覧できるようにして、データは再利用可能、クエリーで絞り込みも容易なmysqlで行うことを想定した環境として作りました。  

pythonは実行環境として考えてはいますがWEBインターフェースとしてflaskをインストールしています。  
ただ、基本コンセプトはpythonを利用してデータ分析と結果を確認で来るための環境のみで各環境は最低限の機能のみとしています。  
